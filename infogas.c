...
int main(void)
{
    ...
    scanf("%f",&gn);
    printf("Distância total a ser percorrida na viagem:\n");
    scanf("%f",&km);
    hn=hi;
    minn=mini;
    odon=odoi;
    ...
    for(i=0;i<n;i++)
    {
        ...
        s=odo-odon;
        temp=s/t;
        printf("Velocidade Média Parcial=%.2fkm/h\n",temp);
        odon=odo;
        hn=h;
        minn=min;
        temp=s/(gn-g);
        ...
        printf("\nVelocidade média final de viagem =%.2fKm/h",temp);
        cmt=s/gn;
        printf("\nConsumo medio total= %.2fKm/l\n\n",cof/n);        
